...
async function recommendModels() {
  ...
  $("emptyState").textContent = "Analyzing document...";
  $("recommendationOutput").innerHTML = "";
  try {
    const profile = await getV4DocumentProfile();
    renderAnalysis();
...
async function getV4DocumentProfile() {
  const file = $("docFile").files[0];
  if (!file) {
    state.analysis = null;
    state.activeProfile = buildDocumentProfile();
    return state.activeProfile;
  }
  const form = new FormData();
  form.append("document", file);
  form.append("document_type", $("docType").value);
  form.append("extraction_preset", $("extractionPreset").value);
  const response = await fetch("/api/v4/analyze", { method: "POST", body: form });
  const json = await response.json();
  if (!response.ok) throw new Error(json.error || "V4 preflight analysis failed");
  state.analysis = json;
  state.activeProfile = json.profile || buildDocumentProfile();
  applyProfileToControls(state.activeProfile);
  return state.activeProfile;
}
...
function applyProfileToControls(profile) {
  ...
  if (profile.page_count) $("pageCount").value = profile.page_count;
  if (profile.text_layer_quality) $("textLayer").value = profile.text_layer_quality;
  if (profile.image_quality) $("imageQuality").value = profile.image_quality;
  if (profile.layout_complexity) $("layoutComplexity").value = profile.layout_complexity;
  if (profile.known_layout_id) $("institutionProfile").value = profile.known_layout_id;
  if (typeof profile.table_density === "number") {
    $("tableDensity").value = profile.table_density >= 0.6 ? "high" : profile.table_density >= 0.3 ? "medium" : "low";
  }
  updateSummary();
}
...
function renderAnalysis() {
  ...
  $("analysisOutput").innerHTML = `
    <div class="analysis-grid">
      ${analysisTile("File type", profile.file_type || "unknown")}
      ${analysisTile("Pages", profile.page_count || 1)}
    ...
    <div class="signal-list">
      <h3>Structural signals used</h3>
      <ul>${(state.analysis.signals || []).map((signal) => `<li>${escapeHtml(signal)}</li>`).join("")}</ul>
    </div>
...
function recommendationSentence(row, index) {
  const profile = state.activeProfile || buildDocumentProfile();
  ...
  if (Number(row.quality_score || 0) >= 0.995) return "Strong fallback with high quality, but lower total score after speed, cost, and tier preference are included.";
...
function buildExplanation(row, index, rows) {
  ...
  const weights = state.decision?.applied_weights || state.decision?.appliedWeights || { cost: 0.35, quality: 0.45, latency: 0.2 };
...
function scoreIntroText(strategy, weights) {
  ...
  return `The ${$("strategy").selectedOptions[0]?.textContent || "Balanced"} goal weights quality ${percent(weights.quality)}, cost ${percent(weights.cost)}, and speed ${percent(weights.latency)}. These weights come from the routing server after the V4 preflight profile is analyzed.`;
}
...
function contextAwareWhy(row, index, leader, profile, preset, rows = []) {
  ...
  if (Number(row.quality_score || 0) >= 0.995 && Number(leader?.quality_score || 0) >= 0.995) {
    return `${rankText} because quality is tied with ${leader?.name || "the top model"}, but its total score is lower after speed, cost, and tier preference are included.`;
  }
