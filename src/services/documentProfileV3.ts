...
export function evaluateDocumentDifficultyV3(prompt: string, profile?: DocumentProfile, explicitTask?: TaskType, stepType?: string): DocumentDifficulty {
  ...
  const institution = institutionSignal(profile);
  if (institution.points) add(institution.points, institution.reason);
...
function institutionSignal(profile?: DocumentProfile) {
  let points = 0;
  let reason = "";
  if (profile?.known_layout_id && knownDifficultLayouts[profile.known_layout_id]) {
    points = knownDifficultLayouts[profile.known_layout_id];
    reason = `known layout ${profile.known_layout_id}`;
  }
  if (profile?.source_institution) {
    for (const [regex, candidatePoints, candidateReason] of institutionDifficulty) {
      if (!regex.test(profile.source_institution)) continue;
